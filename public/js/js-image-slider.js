...
function mcImgSlider(i) {
	for (var I = function(a) {
		...
		}, P = document, J = window.requestAnimationFrame, W = window.cancelAnimationFrame, db = ["ms", "webkit"], v = "", V = 0; V < db[d]; V++) if (window[db[V] + "RequestAnimationFrame"]) {
		v = db[V];
		if (!J) {
	...
	var yb = function() {
		...
		sb = function() {
			var a = yb();
			if (a) if (typeof a.insertRule != "undefined") {
	...
	var G = new L,
		...
		jb = function(b) {
			var a = n[d];
			if (a) while (a--) n[a][g] = a != b && n[a].on == 0 ? "thumb" : "thumb thumb-on"
		},
		ub = function(a) {
		...
		wb = function() {
			var f;
			if (a.l) f = I(a.l);
			if (f) for (var h = U(f, "*"), e = 0; e < h[d]; e++) h[e][g] == "thumb" && n.push(h[e]);
			var c = n[d];
			if (c) {
				while (c--) {
					n[c].on = 0;
	...
	lb.prototype = {
		c: function() {
			...
			while (G--) {
				...
				if (j[k] == "A" && j[g][w]("lazyImage") == -1) if (j[g]) j[g] = "imgLink " + j[g];
				else j[g] = "imgLink";
				if (n) e.push(n);
				else e.push(j);
				if (j[g][w](" video") != -1) {
		...
		x: function() {
			for (var k = [], j = Math.round(o / a.g), i = Math.round(s / a.m), g = 0; g < a.m; g++) for (var f = 0; f < a.g; f++) {
				var d = C("mcBox"),
					e = d[c];
				e.left = j * f + "px";
				e[O] = i * g + "px";
	...
	var Db = function() {
		...
		mb = 0,
		fb = function(e, c) {
			if (++mb < 20) if (!j || typeof tooltip == p) u(function() {
				fb(e, c)
			}, 300);
			else for (var b = t(x), a = 0; a < b[d]; a++) b[a].onmouseover = function() {
